...
struct ExternalCameraDeviceSession : public virtual RefBase, public virtual OutputThreadInterface {
    ExternalCameraDeviceSession(const sp<ICameraDeviceCallback>&, const ExternalCameraConfig& cfg,
    ...
    protected:
        ...
        int cropAndScaleLocked(sp<AllocatedFrame>& in, const Size& outSize, YCbCrLayout* out);

        int cropAndScaleThumbLocked(sp<AllocatedFrame>& in, const Size& outSize, YCbCrLayout* out);
...
protected:
    ...
    Return<void> constructDefaultRequestSettings(
            RequestTemplate, ICameraDeviceSession::constructDefaultRequestSettings_cb _hidl_cb);
    ...
    Return<void> processCaptureRequest(const hidl_vec<CaptureRequest>&,
    ...
    Return<void> configureStreams_3_4(const V3_4::StreamConfiguration& requestedConfiguration,
...
protected:
    ...
    virtual Status importBuffer(int32_t streamId, uint64_t bufId, buffer_handle_t buf,
                                /*out*/ buffer_handle_t** outBufPtr, bool allowEmptyBuf) override;

    virtual Status processCaptureResult(std::shared_ptr<HalRequest>&) override;

    virtual Status processCaptureRequestError(
    ...
    Status configureStreams(
            const V3_2::StreamConfiguration&, V3_3::HalStreamConfiguration* out,
    ...
    virtual Status importRequestLocked(const CaptureRequest& request,
                                       hidl_vec<buffer_handle_t*>& allBufPtrs,
                                       hidl_vec<int>& allFences);

    Status importRequestLockedImpl(const CaptureRequest& request,
                                   hidl_vec<buffer_handle_t*>& allBufPtrs, hidl_vec<int>& allFences,
    ...
    static void cleanupInflightFences(hidl_vec<int>& allFences, size_t numFences);
...
private:
    struct TrampolineSessionInterface_3_4 : public ICameraDeviceSession {
        ...
        virtual Return<void> close() override { return mParent->close(); }
