...
struct pcm_config pcm_config_dsd = {
        ...
        .format = PCM_FORMAT_S32_LE, // This will be converted to SNDRV_PCM_FORMAT_DSD_U32_LE in
                                     // tinyalsa
        .start_threshold = 0,
        .avail_min = 0,
};
...
static void release_buffer(struct resampler_buffer_provider *buffer_provider,
                           struct resampler_buffer *buffer);
static int adev_get_format_for_device(struct imx_audio_device *adev, uint32_t devices,
                                      unsigned int flag);
...
static void convert_passthrough_data_for_s24(void *src, void *dst, unsigned int frames) {
    unsigned int i;
...
static int convert_record_data(void *src, void *dst, unsigned int frames, bool bit_24b_2_16b,
                               bool bit_32b_2_16b, bool mono2stereo, bool stereo2mono) {
    unsigned int i;
...
static void select_mode(struct imx_audio_device *adev) {
    if (adev->mode == AUDIO_MODE_IN_CALL) {
        ...
        ALOGW("Leaving IN_CALL state, in_call=%d, mode=%d", adev->in_call, adev->mode);
        if (adev->in_call) {
            adev->in_call = 0;
            select_output_device(adev);
            select_input_device(adev);
...
static void select_output_device(struct imx_audio_device *adev) {
    ...
    ALOGI("%s(), headphone %d ,headset %d ,speaker %d\n", __func__, headphone_on, headset_on,
          speaker_on);
    /* select output stage */
    for (i = 0; i < adev->audio_card_num; i++)
        set_route_by_array(adev->mixer[i], adev->card_list[i]->headphone_ctl,
...
static int get_card_for_dsd(struct imx_audio_device *adev, int *card_index) {
    ...
    if (card_index && (i != adev->audio_card_num)) *card_index = i;
...
static int get_card_for_bus(struct imx_audio_device *adev, const char *bus, int *p_array_index,
    ...
    if (p_array_index && (i != adev->audio_card_num)) *p_array_index = i;
...
static int get_card_for_device(struct imx_audio_device *adev, int device, unsigned int flag,
    ...
    if (device == AUDIO_DEVICE_NONE) device = AUDIO_DEVICE_OUT_SPEAKER;
...
static int refine_input_parameters(uint32_t *sample_rate, audio_format_t *format,
    ...
    bool inval = false;
    ALOGI("%s: enter: sample_rate(%d) channel_mask(%#x) format(%#x)", __func__, *sample_rate,
          *channel_mask, *format);
...
static void update_passthrough_status(void) {
    if (property_get_int32(PASSTHROUGH_PROPERTY, 0) == PASSTHROUGH_PROPERTY_ENABLE)
...
static int start_output_stream(struct imx_stream_out *out) {
    ...
    if (out->flags & AUDIO_OUTPUT_FLAG_DIRECT) {
        list_for_each(node, &adev->out_streams) {
            struct imx_stream_out *stream = node_to_item(node, struct imx_stream_out, stream_node);
            if ((stream->device == out->device) && (!(stream->flags & AUDIO_OUTPUT_FLAG_DIRECT)) &&
                (!stream->standby)) {
    ...
    if ((!(out->flags & AUDIO_OUTPUT_FLAG_DIRECT)) && (out->device != AUDIO_DEVICE_OUT_BUS)) {
        list_for_each(node, &adev->out_streams) {
            struct imx_stream_out *stream = node_to_item(node, struct imx_stream_out, stream_node);
    ...
    if (lpa_enable) flags |= PCM_LPA;
    if (out->format == AUDIO_FORMAT_DSD) flags |= PCM_FLAG_DSD;
    ...
    ALOGD("%s: pcm_open: card: %d, pcm_device_id: %d, rate: %d, channel: %d, format: %d, "
          "period_size: 0x%x, flag: %x",
          __func__, card, pcm_device_id, config->rate, config->channels, config->format,
          config->period_size, flags);
    ...
    if ((out->flags & AUDIO_OUTPUT_FLAG_COMPRESS_OFFLOAD) && (out->format != AUDIO_FORMAT_DSD)) {
        out->pcm = NULL;
        out->compr = compress_open(card, pcm_device_id, COMPRESS_IN, &out->compr_config);
        ...
        if ((out->flags & AUDIO_OUTPUT_FLAG_PRIMARY) || (out->flags == AUDIO_OUTPUT_FLAG_NONE)) {
            ...
            if (out->buffer == NULL)
                out->buffer = (char *)malloc(
                        out->buffer_frames *
                        audio_stream_out_frame_size(
                                (const struct audio_stream_out *)&out->stream.common));

            if (adev->echo_reference != NULL) out->echo_reference = adev->echo_reference;

            if (out->resampler) out->resampler->reset(out->resampler);
        }

        if (passthrough_for_s24) {
            out->buffer_frames = out->config.period_size * out->config.period_count;
            if (out->buffer == NULL)
                out->buffer = (char *)malloc(
                        out->buffer_frames *
                        audio_stream_out_frame_size(
...
static size_t get_input_buffer_size(uint32_t sample_rate, audio_format_t format,
    ...
    size_t size;
    int channel_count = popcount(channel_mask);
    if (check_input_parameters(sample_rate, format, channel_mask) != 0) return 0;
...
static void add_echo_reference(struct imx_stream_out *out, struct echo_reference_itfe *reference) {
...
static void put_echo_reference(struct imx_audio_device *adev,
    ...
    if (adev->echo_reference != NULL && reference == adev->echo_reference) {
        if (adev->primary_output != NULL && !adev->primary_output->standby)
...
static struct echo_reference_itfe *get_echo_reference(struct imx_audio_device *adev,
    ...
    if (adev->primary_output != NULL && !adev->primary_output->standby) {
        ...
        int status = create_echo_reference(AUDIO_FORMAT_PCM_16_BIT, channel_count, sampling_rate,
                                           AUDIO_FORMAT_PCM_16_BIT, wr_channel_count,
                                           wr_sampling_rate, &adev->echo_reference);
        if (status == 0) add_echo_reference(adev->primary_output, adev->echo_reference);
    }

    return adev->echo_reference;
}

static int get_playback_delay(struct imx_stream_out *out, size_t frames,
    ...
    if (out->pcm) {
        ...
        buffer->delay_ns =
                (long)(((int64_t)(kernel_frames + frames) * 1000000000) / out->sample_rate);
...
static size_t out_get_buffer_size(const struct audio_stream *stream) {
    ...
    // In HAL, AUDIO_FORMAT_DSD doesn't have proportional frames, audio_stream_frame_size will
    // return 1 But in driver, frame_size is 8 byte (DSD_FRAMESIZE_BYTES: 2 channel && 32 bit)
    // Enlarge the buffer size otherwise sound is abnormal when playing high rate DSD stream
    if (out->format == AUDIO_FORMAT_DSD) {
        multiplier = DSD_FRAMESIZE_BYTES * 4;
    }
...
static int do_output_standby(struct imx_stream_out *out, int force_standby) {
    ...
    if ((adev->mode == AUDIO_MODE_IN_CALL) || (adev->b_sco_rx_running == true) ||
        (!force_standby && out->card_index != -1 &&
         !strcmp(adev->card_list[out->card_index]->driver_name, "wm8962-audio"))) {
        ALOGW("no standby");
        return 0;
    }
    if (!out->standby) {
        ...
        if (passthrough_enabled) out->written = 0;
    }
    return 0;
}
...
static int out_dump(const struct audio_stream *stream, int fd) {
    ...
    dprintf(fd, "audio write to ALSA: rate %d, chns %d, alsa format 0x%x\n", out->config.rate,
            out->config.channels, out->config.format);
    dprintf(fd,
            "device 0x%x, card index %d, frames round %d, total written %llu, first_frame_written "
            "%d, writeContiFailCount %d\n",
            out->device, out->card_index, out->frames_round, (long long)out->written,
            out->first_frame_written, out->writeContiFailCount);

    if (out->pcm) dprintf(fd, "pcm fd %d\n", pcm_get_poll_fd(out->pcm));
...
static int out_flush(struct audio_stream_out *stream) {
    ...
    if (out->format == AUDIO_FORMAT_DSD) pcm_flags |= PCM_FLAG_DSD;
    out->pcm = pcm_open(adev->card_list[out->card_index]->card, 0, pcm_flags, &out->config);
    if (out->pcm) out->standby = 0;
...
static char *out_get_parameters(const struct audio_stream *stream, const char *keys) {
    ...
    ALOGW("out get parameters query %s, reply %s", str_parms_to_str(query),
          str_parms_to_str(reply));
...
static uint32_t out_get_latency(const struct audio_stream_out *stream) {
    ...
    if (latency > LPA_LATENCY_MS) {
        ALOGD("%s: Original latency is %dms; Force it to be %dms for LPA", __func__, latency,
              LPA_LATENCY_MS);
...
static int out_set_control_volume(struct mixer *mixer, struct route_setting *route, int left,
                                  int right) {
...
static int pcm_read_convert(struct imx_stream_in *in, struct pcm *pcm, void *data,
                            unsigned int count) {
    ...
    size_t frames_rq =
            count / audio_stream_in_frame_size((const struct audio_stream_in *)&in->stream.common);

    if (in->config.format == PCM_FORMAT_S24_LE && in->requested_format == AUDIO_FORMAT_PCM_16_BIT)
        bit_24b_2_16b = true;
    if (in->config.format == PCM_FORMAT_S32_LE && in->requested_format == AUDIO_FORMAT_PCM_16_BIT)
        bit_32b_2_16b = true;
    ...
    if (bit_24b_2_16b || bit_32b_2_16b || mono2stereo || stereo2mono) {
        ...
        if (in->read_tmp_buf_size < in->config.period_size) {
            ...
            ALOGV("get_next_buffer(): read_tmp_buf %p extended to %zu bytes", in->read_tmp_buf,
                  size_in_bytes_tmp);
        }

        in->read_status =
                pcm_read_wrapper(pcm, (void *)in->read_tmp_buf, size_in_bytes_tmp, in->dump);
        ...
        convert_record_data((void *)in->read_tmp_buf, (void *)data, frames_rq, bit_24b_2_16b,
                            bit_32b_2_16b, mono2stereo, stereo2mono);
    } else {
...
static void audio_dump(const void *buffer, size_t bytes, const char *name) {
    if ((buffer == NULL) || (bytes == 0) || (name == NULL)) return;
...
static int pcm_read_wrapper(struct pcm *pcm, const void *buffer, size_t bytes, bool dump) {
    ...
    if (dump) audio_dump(buffer, bytes, IN_SRC_FILE);
...
static int pcm_write_wrapper(struct pcm *pcm, const void *buffer, size_t bytes, int flags,
    ...
    if (dump) audio_dump(buffer, bytes, OUT_DST_FILE);
...
static ssize_t out_write(struct audio_stream_out *stream, const void *buffer, size_t bytes) {
    ...
    if (out->dump) audio_dump(buffer, bytes, OUT_SRC_FILE);

    // In HAL, AUDIO_FORMAT_DSD doesn't have proportional frames, audio_stream_out_frame_size will
    // return 1 But in driver, frame_size is 8 byte (DSD_FRAMESIZE_BYTES: 2 channel && 32 bit)
    if (out->format == AUDIO_FORMAT_DSD) frame_size *= DSD_FRAMESIZE_BYTES;
    ...
    if ((out->device == AUDIO_DEVICE_OUT_BLUETOOTH_SCO_HEADSET) ||
        ...
        convert_record_data((void *)buffer, (void *)(out->buffer), out_frames, false, false, false,
                            true);
    ...
    if (out->config.channels > 2) convert_output_for_esai(buffer, bytes, out->config.channels);

    if (passthrough_for_s24) {
        convert_passthrough_data_for_s24((void *)buffer, (void *)(out->buffer), out_frames);
    }

    if ((out->flags & AUDIO_OUTPUT_FLAG_COMPRESS_OFFLOAD) && (out->format != AUDIO_FORMAT_DSD)) {
        ALOGV("%s: writing buffer (%zu bytes) to compress device", __func__, bytes);
        ret = compress_write(out->compr, buffer, bytes);
        ALOGV("%s: writing buffer (%zu bytes) to compress device returned %d", __func__, bytes,
              ret);
        ...
        if (((out->flags & AUDIO_OUTPUT_FLAG_PRIMARY) || (out->flags == AUDIO_OUTPUT_FLAG_NONE)) &&
            ...
            ret = pcm_write_wrapper(out->pcm, (void *)out->buffer, out_frames * frame_size,
                                    out->write_flags, out->dump);
            ...
            ret = pcm_write_wrapper(out->pcm, (void *)out->buffer, out_frames * frame_size * 2,
                                    out->write_flags, out->dump);
        } else {
...
/********************************************************************************************************
For esai, it will use the first channels/2 for Left channels, use second half channels for Right
channels. So we need to transform channel map in HAL for multichannel. when input is "FL, FR, C,
LFE, BL, BR, SL, SR", output is "FL,BL,C,FR,BR,LFE". when input is "FL, FR, C, LFE, BL, BR, SL, SR",
output is "FL,BL,C,SL,FR,BR,LFE,SR"
*********************************************************************************************************/
static void convert_output_for_esai(const void *buffer, size_t bytes, int channels) {
...
static int out_get_presentation_position(const struct audio_stream_out *stream, uint64_t *frames,
    ...
    if ((out->flags & AUDIO_OUTPUT_FLAG_COMPRESS_OFFLOAD) && (out->format != AUDIO_FORMAT_DSD)) {
        if (out->compr != NULL) {
            ret = compress_get_tstamp(out->compr, &dsp_frames, &out->sample_rate);
            if (ret) ALOGE("%s: error to get tstamp: %d", __func__, ret);
            ALOGV("%s rendered frames %ld sample_rate %d", __func__, dsp_frames, out->sample_rate);
            *frames = dsp_frames;
            ret = 0;
        ...
        if (pcm_get_htimestamp(out->pcm, &avail, timestamp) == 0) {
            ...
            if (out->format == AUDIO_FORMAT_DSD)
                // In AudioFlinger, frame_size is 1/4 byte(2 channel && 1 bit), here it is 8 byte(2
                // channel && 32 bit)
            ...
            ALOGV("%s: avail: %u kernel_buffer_size: %zu written: %lld signed_frames: %ld",
                  __func__, avail, kernel_buffer_size, (long long)out->written,
                  (long)signed_frames);
    ...
    if (!strcmp(adev->card_list[i]->driver_name, "imx-spdif")) {
        ...
        if (rate > 0 && rate != in->config.rate) {
            ...
            if (in->requested_rate != in->config.rate) {
                ...
                ret = create_resampler(in->config.rate, in->requested_rate, in->requested_channel,
                                       RESAMPLER_QUALITY_DEFAULT, &in->buf_provider,
                                       &in->resampler);
            }
...
static int start_input_stream(struct imx_stream_in *in) {
    ...
    if (in->need_echo_reference && in->echo_reference == NULL)
        in->echo_reference = get_echo_reference(adev, AUDIO_FORMAT_PCM_16_BIT,
                                                in->requested_channel, in->requested_rate);
...
static size_t in_get_buffer_size(const struct audio_stream *stream) {
    ...
    return get_input_buffer_size(in->requested_rate, in->requested_format,
...
static int in_dump(const struct audio_stream *stream, int fd) {
    ...
    dprintf(fd, "audio read from HAL: rate %d, chns %d, audio format 0x%x\n", in->requested_rate,
            popcount(in->requested_channel), in->requested_format);
    dprintf(fd, "audio read from ALSA: rate %d, chns %d, alsa format 0x%x\n", in->config.rate,
            in->config.channels, in->config.format);
    dprintf(fd,
            "device 0x%x, card index %d, frames round %d, first_frame_read %d, read_status %d\n",
            in->device, in->card_index, in->frames_round, in->first_frame_read, in->read_status);

    if (in->pcm) dprintf(fd, "pcm fd %d\n", pcm_get_poll_fd(in->pcm));
...
static int in_set_parameters(struct audio_stream *stream, const char *kvpairs) {
    ...
    if (do_standby) do_input_standby(in);
...
static char *in_get_parameters(const struct audio_stream *stream __unused, const char *keys) {
    struct str_parms *query = str_parms_create_str(keys);
    char *str = NULL;
    char value[256];
    struct str_parms *reply = str_parms_create();
...
static void get_capture_delay(struct imx_stream_in *in, size_t frames,
                              struct echo_reference_buffer *buffer) {
    ...
    ALOGV("get_capture_delay time_stamp = [%ld].[%ld], delay_ns: [%d],"
          ...
          buffer->time_stamp.tv_sec, buffer->time_stamp.tv_nsec, buffer->delay_ns, kernel_delay,
          buf_delay, rsmp_delay, kernel_frames, in->read_buf_frames, in->proc_buf_frames, frames);
}
...
static int32_t update_echo_reference(struct imx_stream_in *in, size_t frames) {
    ...
    if (in->ref_frames_in < frames) {
        if (in->ref_buf_size < frames) {
            ...
            in->ref_buf =
                    (int16_t *)realloc(in->ref_buf,
                                       in->ref_buf_size * in->requested_channel * sizeof(int16_t));
        }
...
static int set_preprocessor_param(effect_handle_t handle, effect_param_t *param) {
    uint32_t size = sizeof(int);
    uint32_t psize = ((param->psize - 1) / sizeof(int) + 1) * sizeof(int) + param->vsize;

    int status = (*handle)->command(handle, EFFECT_CMD_SET_PARAM, sizeof(effect_param_t) + psize,
                                    param, &size, &param->status);
    if (status == 0) status = param->status;
...
static void push_echo_reference(struct imx_stream_in *in, size_t frames) {
    ...
    if (in->ref_frames_in < frames) frames = in->ref_frames_in;

    buf.frameCount = frames;
    buf.raw = in->ref_buf;

    for (i = 0; i < in->num_preprocessors; i++) {
        if ((*in->preprocessors[i].effect_itfe)->process_reverse == NULL) continue;

        (*in->preprocessors[i].effect_itfe)
                ->process_reverse(in->preprocessors[i].effect_itfe, &buf, NULL);
        set_preprocessor_echo_delay(in->preprocessors[i].effect_itfe, delay_us);
    }

    in->ref_frames_in -= buf.frameCount;
    if (in->ref_frames_in) {
        memcpy(in->ref_buf, in->ref_buf + buf.frameCount * in->requested_channel,
...
static int get_next_buffer(struct resampler_buffer_provider *buffer_provider,
    ...
    if (in->read_buf_frames == 0) {
        size_t size_in_bytes = in->config.period_size *
                audio_stream_in_frame_size((const struct audio_stream_in *)&in->stream.common);
        if (in->read_buf_size < in->config.period_size) {
            ...
            ALOG_ASSERT((in->read_buf != NULL), "get_next_buffer() failed to reallocate read_buf");
            ALOGV("get_next_buffer(): read_buf %p extended to %zu bytes", in->read_buf,
                  size_in_bytes);
        }
    ...
    buffer->frame_count =
            (buffer->frame_count > in->read_buf_frames) ? in->read_buf_frames : buffer->frame_count;
    buffer->i16 =
            in->read_buf + (in->config.period_size - in->read_buf_frames) * in->requested_channel;
...
static ssize_t read_frames(struct imx_stream_in *in, void *buffer, ssize_t frames) {
    ...
    while (frames_wr < frames) {
        ...
        if (in->resampler != NULL) {
            in->resampler
                    ->resample_from_provider(in->resampler,
                                             (int16_t *)((char *)buffer +
                                                         frames_wr *
            ...
            if (buf.raw != NULL) {
                memcpy((char *)buffer +
                               frames_wr *
                                       audio_stream_in_frame_size(
                                               (const struct audio_stream_in *)&in->stream.common),
                       buf.raw,
                       buf.frame_count *
                               audio_stream_in_frame_size(
                                       (const struct audio_stream_in *)&in->stream.common));
...
static ssize_t process_frames(struct imx_stream_in *in, void *buffer, ssize_t frames) {
    ...
    while (frames_wr < frames) {
        ...
        if (in->proc_buf_frames < (size_t)frames) {
            ...
            if (in->proc_buf_size < (size_t)frames) {
                ...
                in->proc_buf_in = (int16_t *)realloc(in->proc_buf_in,
                                                     in->proc_buf_size * in->requested_channel *
                                                             sizeof(int16_t));
                ...
                ALOGV("process_frames(): proc_buf_in %p extended to %zu bytes", in->proc_buf_in,
                      in->proc_buf_size * in->requested_channel * sizeof(int16_t));
            }
            frames_rd =
                    read_frames(in, in->proc_buf_in + in->proc_buf_frames * in->requested_channel,
        ...
        if (in->echo_reference != NULL) push_echo_reference(in, in->proc_buf_frames);
        ...
        for (i = 0; i < in->num_preprocessors; i++) {
            (*in->preprocessors[i].effect_itfe)
                    ->process(in->preprocessors[i].effect_itfe, &in_buf, &out_buf);
        }
        ...
        if (in->proc_buf_frames) {
            memcpy(in->proc_buf_in, in->proc_buf_in + in_buf.frameCount * in->requested_channel,
...
static ssize_t in_read(struct audio_stream_in *stream, void *buffer, size_t bytes) {
    ...
    struct imx_audio_device *adev = in->dev;
    size_t frames_rq =
            bytes / audio_stream_in_frame_size((const struct audio_stream_in *)&stream->common);
    ...
    if (in->standby) {
        ...
        if (ret == 0) {
            in->standby = 0;
            in->mute_500ms = in->requested_rate *
                    audio_stream_in_frame_size((const struct audio_stream_in *)&stream->common) / 2;
        }
    }
    pthread_mutex_unlock(&adev->lock);
...
exit:
    ...
    if (ret < 0) {
        ...
        usleep(bytes * 1000000 /
               audio_stream_in_frame_size((const struct audio_stream_in *)&stream->common) /
    ...
    if (in->dump) audio_dump(buffer, bytes, IN_DST_FILE);
...
static int in_get_capture_position(const struct audio_stream_in *stream, int64_t *frames,
    ...
    pthread_mutex_lock(&in->lock);
    if (in->standby) {
        ALOGE_IF(in->pcm != NULL, "%s stream in standby but pcm not NULL for non ST session",
                 __func__);
        goto exit;
    }
    if (in->pcm) {
...
static int in_configure_effect(struct imx_stream_in *in, effect_handle_t effect) {
    ...
    fct_status = (*(effect))->command(effect, EFFECT_CMD_SET_CONFIG, sizeof(effect_config_t),
                                      &config, &size, &cmd_status);
...
static int in_configure_reverse(struct imx_stream_in *in) {
    ...
    if (in->num_preprocessors > 0) {
        ...
        for (i = 0; i < in->num_preprocessors; i++) {
            if ((*in->preprocessors[i].effect_itfe)->process_reverse == NULL) continue;
            fct_status = (*(in->preprocessors[i].effect_itfe))
                                 ->command(in->preprocessors[i].effect_itfe,
                                           EFFECT_CMD_SET_CONFIG_REVERSE, sizeof(effect_config_t),
                                           &config, &size, &cmd_status);
            GET_COMMAND_STATUS(status, fct_status, cmd_status);
        }
    }
...
static void in_read_audio_effect_channel_configs(struct imx_stream_in *in __unused,
    ...
    cmd_status = (*effect)->command(effect, EFFECT_CMD_GET_FEATURE_SUPPORTED_CONFIGS, cmd_size,
                                    (void *)&cmd, &reply_size, (void *)&reply);

    if (cmd_status != 0) {
        ALOGV("in_read_audio_effect_channel_configs(): "
              "fx->command returned %d",
              cmd_status);
        return;
    }
    ...
    ALOGV("in_read_audio_effect_channel_configs()(): "
          "Feature supported and adding %d channel configs to the list",
          reply[1]);
...
static audio_channel_mask_t in_get_aux_channels(struct imx_stream_in *in) {
    ...
    if (in->num_preprocessors == 0) return AUDIO_CHANNEL_NONE;
        ...
        if (cur_chcfg->main_channels == in->main_channels) {
            ...
            for (idx_preproc = 0, match_cnt = 0;
                ...
                for (idx_chcfg = 0; idx_chcfg < effect_info->num_channel_configs; idx_chcfg++) {
                    if (memcmp(effect_info->channel_configs + idx_chcfg, cur_chcfg,
...
static int in_configure_effect_channels(effect_handle_t effect, channel_config_t *channel_config) {
    int status = 0;
    int fct_status;
    int32_t cmd_status;
    uint32_t reply_size;
    ...
    ALOGV("in_configure_effect_channels(): configure effect with channels: [%04x][%04x]",
          channel_config->main_channels, channel_config->aux_channels);

    config.inputCfg.mask = EFFECT_CONFIG_CHANNELS;
    config.outputCfg.mask = EFFECT_CONFIG_CHANNELS;
    reply_size = sizeof(effect_config_t);
    fct_status = (*effect)->command(effect, EFFECT_CMD_GET_CONFIG, 0, NULL, &reply_size, &config);
    if (fct_status != 0) {
    ...
    fct_status = (*effect)->command(effect, EFFECT_CMD_SET_CONFIG, sizeof(effect_config_t), &config,
                                    &reply_size, &cmd_status);
    GET_COMMAND_STATUS(status, fct_status, cmd_status);

    cmd[0] = EFFECT_FEATURE_AUX_CHANNELS;
    memcpy(cmd + 1, channel_config, sizeof(channel_config_t));
    reply_size = sizeof(uint32_t);
    fct_status = (*effect)->command(effect, EFFECT_CMD_SET_FEATURE_CONFIG,
                                    sizeof(cmd), // sizeof(uint32_t) + sizeof(channel_config_t),
                                    cmd, &reply_size, &cmd_status);
    GET_COMMAND_STATUS(status, fct_status, cmd_status);

    /* some implementations need to be re-enabled after a config change */
    reply_size = sizeof(uint32_t);
    fct_status = (*effect)->command(effect, EFFECT_CMD_ENABLE, 0, NULL, &reply_size, &cmd_status);
    GET_COMMAND_STATUS(status, fct_status, cmd_status);

    return status;
}

static int in_reconfigure_channels(struct imx_stream_in *in, effect_handle_t effect,
                                   channel_config_t *channel_config, bool config_changed) {
    int status = 0;

    ALOGV("in_reconfigure_channels(): config_changed %d effect %p", config_changed, effect);
    ...
    if (config_changed) {
        ...
        for (i = 0; i < in->num_preprocessors; i++) {
            int cur_status =
                    in_configure_effect_channels(in->preprocessors[i].effect_itfe, channel_config);
            if (cur_status != 0) {
                ALOGV("in_reconfigure_channels(): error %d configuring effect "
                      "%d with channels: [%04x][%04x]",
                      cur_status, i, channel_config->main_channels, channel_config->aux_channels);
                status = cur_status;
            }
        }
...
static void in_update_aux_channels(struct imx_stream_in *in, effect_handle_t effect) {
    ...
    status = in_reconfigure_channels(in, effect, &channel_config,
...
static int adev_set_audio_port_config(struct audio_hw_device *dev,
    ...
    struct imx_stream_out *out =
            (struct imx_stream_out *)hashmapGet(adev->out_bus_stream_map, bus_address);
    int card_index = -1;
...
struct imx_stream_out *get_stream_out_by_io_handle_l(struct imx_audio_device *adev,
                                                     audio_io_handle_t handle) {
    struct listnode *node;

    list_for_each(node, &adev->out_streams) {
        struct imx_stream_out *out = node_to_item(node, struct imx_stream_out, stream_node);
...
// This must be called with adev->lock held.
struct imx_stream_in *get_stream_in_by_io_handle_l(struct imx_audio_device *adev,
                                                   audio_io_handle_t handle) {
    struct listnode *node;

    list_for_each(node, &adev->in_streams) {
        struct imx_stream_in *in = node_to_item(node, struct imx_stream_in, stream_node);
        if (in->handle == handle) {
            return in;
        }
    }
...
// This must be called with adev->lock held.
struct imx_stream_out *get_stream_out_by_patch_handle_l(struct imx_audio_device *adev,
...
// This must be called with adev->lock held.
struct imx_stream_in *get_stream_in_by_patch_handle_l(struct imx_audio_device *adev,
                                                      audio_patch_handle_t patch_handle) {
    struct listnode *node;
...
static int adev_create_audio_patch(struct audio_hw_device *dev, unsigned int num_sources,
    ...
    if (sources[0].type == AUDIO_PORT_TYPE_DEVICE) {
        ALOGD("%s() device->mix: %d -> %d", __func__,
              sources[0].ext.device.type & ~AUDIO_DEVICE_BIT_IN, sinks[0].ext.mix.handle);
        ...
        ALOGD("%s() mix->device: %d -> %d", __func__, sources[0].ext.mix.handle,
              sinks[0].ext.device.type);
    ...
    if (sources[0].type == AUDIO_PORT_TYPE_DEVICE) {
        struct imx_stream_in *in = get_stream_in_by_io_handle_l(adev, sinks[0].ext.mix.handle);
        if (in == NULL) {
        ...
        if (in->device != sources[0].ext.device.type) {
            ...
            do_input_standby(in);
            in_update_aux_channels(in, NULL);
            ALOGD("%s() set input(%d) device to %x, patch_handle %d", __func__, in->handle,
                  in->device, *handle);
        }
        pthread_mutex_unlock(&in->lock);
        in->patch_handle = *handle;
    } else {
        struct imx_stream_out *out = get_stream_out_by_io_handle_l(adev, sources[0].ext.mix.handle);
        if (out == NULL) {
            ALOGE("%s()can not find stream with handle(%d)", __func__, sources[0].ext.mix.handle);
            ret = -EINVAL;
            goto error;
        ...
        if (out->device != sinks[0].ext.device.type) {
            ...
            do_output_standby(out, true);
            ALOGD("%s() set output(%d) device to %x, patch_handle %d", __func__, out->handle,
                  out->device, *handle);
        }
        pthread_mutex_unlock(&out->lock);
...
static int adev_open_output_stream(struct audio_hw_device *dev, audio_io_handle_t handle __unused,
    ...
    if ((flags & AUDIO_OUTPUT_FLAG_COMPRESS_OFFLOAD) && (out->format != AUDIO_FORMAT_DSD)) {
        ...
        out->compr_config.codec->ch_in = audio_channel_count_from_out_mask(out->channel_mask);
        out->compr_config.codec->ch_out = out->compr_config.codec->ch_in;
        ALOGI("%s: offloaded output offload_info version %04x bit rate %d", __func__,
              config->offload_info.version, config->offload_info.bit_rate);
        ...
        out->stream.flush = out_flush;
    } else if (flags & AUDIO_OUTPUT_FLAG_DIRECT && devices == AUDIO_DEVICE_OUT_AUX_DIGITAL) {
        ALOGW("adev_open_output_stream() HDMI multichannel");
        ret = out_read_hdmi_channel_masks(ladev, out);
        if (ret != 0) goto err_open;
        ...
        if (strcmp(ladev->device_name, "evk_8mp") == 0 ||
            ...
            if (passthrough_enabled) {
               ...
               ((devices == AUDIO_DEVICE_OUT_SPEAKER) || (devices == AUDIO_DEVICE_OUT_LINE) ||
        ...
        ALOGD("%s: LPA esai direct output stream, hold_second: %d, period_ms: %d", __func__,
              lpa_hold_second, lpa_period_ms);
        if (config->sample_rate == 0) {
            config->sample_rate = DEFAULT_OUTPUT_SAMPLE_RATE;
            out->sample_rate = config->sample_rate;
        }
        ...
        if (flags & AUDIO_OUTPUT_FLAG_PRIMARY) {
            if (ladev->primary_output == NULL)
                ...
                if (out->device == AUDIO_DEVICE_OUT_BUS)
                    ALOGD("%s: car device already has primary output: %p", __func__,
                          ladev->primary_output);
                else {
    ...
    if ((config->format != AUDIO_FORMAT_DEFAULT &&
        ...
        ALOGE("%s: Unsupported output config. sample_rate:%d:%d; format:%#x:%#x; "
              "channel_mask:%#x:%#x",
              __func__, config->sample_rate,
              out->stream.common.get_sample_rate(&out->stream.common), config->format,
              out->stream.common.get_format(&out->stream.common), config->channel_mask,
...
static void adev_close_output_stream(struct audio_hw_device *dev, struct audio_stream_out *stream) {
    ...
    if ((out->flags & AUDIO_OUTPUT_FLAG_COMPRESS_OFFLOAD) && (out->format != AUDIO_FORMAT_DSD)) {
        if (out->compr_config.codec != NULL) free(out->compr_config.codec);
    }
...
static void *sco_rx_task(void *arg) {
    ...
    sco_rx_out_buffer_size = stream_out->buffer_frames *
            audio_stream_out_frame_size(
                                     (const struct audio_stream_out *)&stream_out->stream.common);
    ...
    while (adev->b_sco_rx_running) {
        ...
        if (ret) {
            ALOGE("sco_rx_task, pcm_read ret %d, size %d, %s", ret, size,
                  pcm_get_error(adev->pcm_sco_rx));
            usleep(2000);
            continue;
        }
...
static void *sco_tx_task(void *arg) {
    ...
    while (adev->b_sco_tx_running) {
        ...
        if (ret) {
            ALOGI("sco_tx_task, pcm_read ret %d, size %d, %s", ret, size,
                  pcm_get_error(adev->pcm_cap));
            continue;
        }
        if (mono_buffer) {
            /* Convert to mono */
            ret = adjust_channels(buffer, adev->cap_config.channels, mono_buffer, 1,
        ...
        out_frames = pcm_config_sco_out.period_size;
        adev->rsmpl_sco_tx->resample_from_input(adev->rsmpl_sco_tx, p_mono_buffer,
                                                (size_t *)&frames, (int16_t *)out_buffer,
                                                (size_t *)&out_frames);

        ALOGV("sco_tx_task, resample_from_input, in frames %d, %d, out_frames %d, %d",
              adev->cap_config.period_size, frames, pcm_config_sco_out.period_size, out_frames);

        out_size = pcm_frames_to_bytes(adev->pcm_sco_tx, out_frames);
        ret = pcm_write(adev->pcm_sco_tx, out_buffer, out_size);
        if (ret) {
            ALOGE("sco_tx_task, pcm_write ret %d, size %d, %s", ret, out_size,
                  pcm_get_error(adev->pcm_sco_tx));
        }
    }
...
static int sco_task_create(struct imx_audio_device *adev) {
    ...
    pcm_config_sco_in.period_size =
            pcm_config_mm_out.period_size * pcm_config_sco_in.rate / pcm_config_mm_out.rate;
    ...
    ALOGI("rate %d, channel %d, period_size 0x%x, period_count %d", pcm_config_sco_in.rate,
          pcm_config_sco_in.channels, pcm_config_sco_in.period_size,
          pcm_config_sco_in.period_count);
    ...
    ALOGI("after pcm open, rate %d, channel %d, period_size 0x%x, period_count %d",
          pcm_config_sco_in.rate, pcm_config_sco_in.channels, pcm_config_sco_in.period_size,
          pcm_config_sco_in.period_count);
    ...
    // create resampler
    ret = create_resampler(pcm_config_sco_in.rate, pcm_config_mm_out.rate,
                           pcm_config_sco_in.channels, RESAMPLER_QUALITY_DEFAULT, NULL,
                           &adev->rsmpl_sco_rx);
    if (ret) {
    ...
    ALOGI("create_resampler rsmpl_sco_rx, in rate %d, out rate %d", pcm_config_sco_in.rate,
    ...
    ALOGI("rate %d, channel %d, period_size 0x%x", pcm_config_sco_out.rate,
          pcm_config_sco_out.channels, pcm_config_sco_out.period_size);
    ...
    adev->cap_config.period_size =
            pcm_config_sco_out.period_size * adev->cap_config.rate / pcm_config_sco_out.rate;
    ALOGW(" open mic, card %d, port %d", card, port);
    ALOGW("rate %d, channel %d, period_size 0x%x", adev->cap_config.rate, adev->cap_config.channels,
          adev->cap_config.period_size);
    ...
    // create resampler
    ret = create_resampler(adev->cap_config.rate, pcm_config_sco_out.rate,
                           pcm_config_sco_out.channels, RESAMPLER_QUALITY_DEFAULT, NULL,
                           &adev->rsmpl_sco_tx);
    if (ret) {
    ...
    ALOGI("create_resampler rsmpl_sco_tx, in rate %d, out rate %d", adev->cap_config.rate,
...
static int adev_set_parameters(struct audio_hw_device *dev, const char *kvpairs) {
    ...
    if (ret >= 0) {
        ...
        if (tty_mode != adev->tty_mode) {
            ...
            if (adev->mode == AUDIO_MODE_IN_CALL) select_output_device(adev);
        }
        pthread_mutex_unlock(&adev->lock);
    }
...
static int adev_open_input_stream(struct audio_hw_device *dev, audio_io_handle_t handle __unused,
                                  audio_devices_t devices, struct audio_config *config,
                                  struct audio_stream_in **stream_in,
                                  audio_input_flags_t flags __unused, const char *address,
...
static void adev_close_input_stream(struct audio_hw_device *dev, struct audio_stream_in *stream) {
    ...
    if (in->read_buf) free(in->read_buf);

    if (in->resampler) {
        release_resampler(in->resampler);
        in->resampler = NULL;
    }
    if (in->proc_buf_in) free(in->proc_buf_in);
    if (in->proc_buf_out) free(in->proc_buf_out);
    if (in->ref_buf) free(in->ref_buf);
    if (in->address) free(in->address);
...
static int adev_close(hw_device_t *device) {
    ...
    if (!device || ((struct imx_audio_device *)device != adev)) goto done;

    if ((--audio_device_ref_count) == 0) {
        for (i = 0; i < adev->audio_card_num; i++)
            if (adev->mixer[i]) mixer_close(adev->mixer[i]);
...
static void adjust_card_sequence(struct imx_audio_device *adev) {
    ...
    if ((adev == NULL) || (adev->audio_card_num == 0)) return;
    ...
    if ((pcard_wm8960 == NULL) || (pcard_cs42888 == NULL)) return;

    if (idx_cs42888 < idx_wm8960) return;
...
static void audio_card_refine_config(struct audio_card *audio_card, struct imx_audio_device *adev) {
    if (!audio_card || !adev) return;

    int card = audio_card->card;

    if (strstr(audio_card->driver_name, "sco-audio")) g_hsp_chns = 1;
    ...
    // The default period size:count is 192:8. On some cards/platforms(rpmsg/imx7ulp), it's too
    // small. Will Underrun due to the producer (MonoPipe::write) is schduled at lower frequency.
    // Fix me, pcm_config_mm_out will impact all the cards on one board.
    if (audio_card->out_period_size > 0) {
        ALOGI("%s, set out period_size to %d", audio_card->driver_name,
              audio_card->out_period_size);
        pcm_config_mm_out.period_size = audio_card->out_period_size;
    }

    if (audio_card->out_period_count > 0) {
        ALOGI("%s, set out period_count to %d", audio_card->driver_name,
              audio_card->out_period_count);
    ...
    if (audio_card->in_period_count > 0) {
        ALOGI("%s, set in period_count to %d", audio_card->driver_name,
              audio_card->in_period_count);
...
static int adev_open(const hw_module_t *module, const char *name, hw_device_t **device) {
    ...
    property_get(PRODUCT_DEVICE_PROPERTY, adev->device_name, DEFAULT_ERROR_NAME_str);
