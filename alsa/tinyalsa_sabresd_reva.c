...
#define CARD_IMX_MAIN "wm8958-audio"
                     ...
                     hw_device_t** device)
{
    ...
    if(adev->main_card >=0 )
    {
        ...
        adev->mixer_ctls.headset_enable = mixer_get_ctl_by_name(adev->mixer,
                                           MIXER_WM8958_HEADPHONE_SWITCH);
        adev->mixer_ctls.speaker_enable = mixer_get_ctl_by_name(adev->mixer,
                                           MIXER_WM8958_SPEAKER_SWITCH);
        adev->mixer_ctls.earpiece_enable = mixer_get_ctl_by_name(adev->mixer,
                                           MIXER_WM8958_EARPIECE_SWITCH);
        adev->mixer_ctls.headset_volume = mixer_get_ctl_by_name(adev->mixer,
                                           MIXER_WM8958_HEADPHONE_VOLUME);
        adev->mixer_ctls.speaker_volume = mixer_get_ctl_by_name(adev->mixer,
                                           MIXER_WM8958_SPEAKER_VOLUME);
        adev->mixer_ctls.earpiece_volume = mixer_get_ctl_by_name(adev->mixer,
                                           MIXER_WM8958_EARPIECE_VOLUME);
        if (!adev->mixer_ctls.headset_enable || !adev->mixer_ctls.speaker_enable ||
