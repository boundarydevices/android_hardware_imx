...
struct CameraSensorMetadata {
    ...
    // Ref
    // https://developer.android.com/reference/android/hardware/camera2/CameraCharacteristics#CONTROL_AE_COMPENSATION_RANGE
    // and
    ...
    // Ref above link, One unit of EV compensation changes the brightness of the captured image by a
    // factor of two. +1 EV doubles the image brightness, while -1 EV halves the image brightness.
    int mAeCompMin;
    int mAeCompMax;
...
typedef std::unordered_map<uint32_t,
                           std::pair<CameraDeviceStatus, std::unique_ptr<CameraSensorMetadata>>>
        PhysicalDeviceMap;
