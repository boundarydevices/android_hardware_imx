...
status_t CameraDeviceHwlImpl::Initialize() {
    ...
    if (mSensorData.mAvailableCapabilities ==
        ANDROID_REQUEST_AVAILABLE_CAPABILITIES_LOGICAL_MULTI_CAMERA) {
        uint8_t logical_available_capabilities[] =
                {ANDROID_REQUEST_AVAILABLE_CAPABILITIES_BACKWARD_COMPATIBLE,
                 ANDROID_REQUEST_AVAILABLE_CAPABILITIES_LOGICAL_MULTI_CAMERA};
        (m_meta->GetStaticMeta())
                ->Set(ANDROID_REQUEST_AVAILABLE_CAPABILITIES, logical_available_capabilities,
        ...
        (m_meta->GetStaticMeta())
                ->Set(ANDROID_LOGICAL_MULTI_CAMERA_SENSOR_SYNC_TYPE, &logical_sensor_sync_type, 1);
        ...
        for (const auto &physical_device : *(physical_device_map_)) {
            ...
            physical_ids.insert(physical_ids.end(), physical_id.begin(), physical_id.end());
            ...
            physical_meta_map_.emplace(physical_device.first,
                                       HalCameraMetadata::Clone(m_phy_meta->GetStaticMeta()));
        ...
        (m_meta->GetStaticMeta())
                ->Set(ANDROID_LOGICAL_MULTI_CAMERA_PHYSICAL_IDS, physical_ids.data(),
                      physical_ids.size());

        // Additionally if possible try to emulate a logical camera device backed by
        // physical devices with different focal lengths.
        // Usually logical cameras will have device specific logic to switch between physical
        // sensors.
        ...
        focal_buffer.insert(focal_buffer.end(), focal_lengths.begin(), focal_lengths.end());
        (m_meta->GetStaticMeta())
                ->Set(ANDROID_LENS_INFO_AVAILABLE_FOCAL_LENGTHS, focal_buffer.data(),
                      focal_buffer.size());
        ...
        if (ret == OK) {
            ...
            (m_meta->GetStaticMeta())
                    ->Set(ANDROID_REQUEST_AVAILABLE_RESULT_KEYS, keys_buffer.data(),
                          keys_buffer.size());

            keys.clear();
            keys_buffer.clear();
            auto ret = (m_meta->GetStaticMeta())
                               ->Get(ANDROID_REQUEST_AVAILABLE_CHARACTERISTICS_KEYS, &entry);
            if (ret == OK) {
                ...
                (m_meta->GetStaticMeta())
                        ->Erase(ANDROID_REQUEST_AVAILABLE_PHYSICAL_CAMERA_REQUEST_KEYS);
                ...
                (m_meta->GetStaticMeta())
                        ->Set(ANDROID_REQUEST_AVAILABLE_CHARACTERISTICS_KEYS, keys_buffer.data(),
                              keys_buffer.size());
            }
            ...
            if (ret == OK) {
                ...
                keys_buffer.insert(keys_buffer.end(), keys.begin(), keys.end());
                (m_meta->GetStaticMeta())
                        ->Set(ANDROID_REQUEST_AVAILABLE_REQUEST_KEYS, keys_buffer.data(),
                              keys_buffer.size());
            }
        }
    }
