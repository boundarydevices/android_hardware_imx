...
int EncOutFrameBufCreateRegisterFrame(VpuCodStd eFormat, int nInColor,
    ...
    ALOGV("%s, EOFBCRF, eFormat %d, nInColor %d, nInAlign %d, nInMapType %d", __func__, eFormat,
          nInColor, nInAlign, nInMapType);
...
int EncMallocMemBlock(VpuMemInfo *pMemBlock, EncMemInfo *pEncMem) {
    ...
    for (i = 0; i < pMemBlock->nSubBlockNum; i++) {
        ALOGV("EncMallocMemBlock, i %d, align %d, size %d, memType %d", i,
              pMemBlock->MemSubBlock[i].nAlignment, pMemBlock->MemSubBlock[i].nSize,
...
int vpu_encode(void *inYuv, void *inYuvPhy, int Width, int Height, int /*quality*/, int color,
    ...
    ALOGV("%s, Init OK: min buffer cnt: %d, alignment: %d", __func__,
    ...
    if ((sEncEncParam.eOutRetCode & VPU_ENC_OUTPUT_DIS) ||
        ...
        ALOGE("%s, vpu encode frame failure: no output,  ret=0x%X ", __func__,
...
int YuvToJpegEncoder::yuvResize(uint8_t *srcBuf, int srcWidth, int srcHeight, uint8_t *dstBuf,
    ...
    return yuv422iResize(srcBuf, srcWidth, srcHeight, dstBuf, dstWidth, dstHeight);
}

// /////////////////////////////////////////////////////////////////
Yuv420SpToJpegEncoder::Yuv420SpToJpegEncoder(int format) : YuvToJpegEncoder(format) {
    fNumPlanes = 2;
    color = 0;
#ifdef BOARD_HAVE_VPU
    supportVpu = true;
...
int Yuv420SpToJpegEncoder::yuvResize(uint8_t *srcBuf, int srcWidth, int srcHeight, uint8_t *dstBuf,
                                     int dstWidth, int dstHeight) {
    return yuv420spResize(srcBuf, srcWidth, srcHeight, dstBuf, dstWidth, dstHeight);
}

// /////////////////////////////////////////////////////////////////////////////
Yuv422IToJpegEncoder::Yuv422IToJpegEncoder(int format) : YuvToJpegEncoder(format) {
    fNumPlanes = 1;
    color = 1;
#ifdef BOARD_HAVE_VPU
...
// /////////////////////////////////////////////////////////////////////////////////////////////
Yuv422SpToJpegEncoder::Yuv422SpToJpegEncoder(int format) : YuvToJpegEncoder(format) {
    fNumPlanes = 1;
    color = 1;
#ifdef BOARD_HAVE_VPU
    supportVpu = true;
