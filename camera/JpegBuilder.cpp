...
status_t JpegBuilder::prepareImage(const ImxStreamBuffer *streamBuf)
{
    status_t ret = NO_ERROR;
    struct timeval sTv;
    struct tm     *pTime;
    ...
    ImxStream *stream = streamBuf->mStream;
...
status_t JpegBuilder::buildImage(ImxStreamBuffer *streamBuf)
{
    int ret = 0;
