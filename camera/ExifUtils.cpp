...
protected:
    ...
    float ConvertToApex(float val) { return 2.0f * log2f(val); }
...
bool ExifUtilsImpl::SetDigitalZoomRatio(uint32_t crop_width, uint32_t crop_height,
                                        uint32_t sensor_width, uint32_t sensor_height) {
    float zoom_ratio_x = (crop_width == 0) ? 1.0 : 1.0 * sensor_width / crop_width;
    float zoom_ratio_y = (crop_height == 0) ? 1.0 : 1.0 * sensor_height / crop_height;
...
bool ExifUtilsImpl::SetFocalLengthIn35mmFilm(float focal_length, float sensor_size_x,
                                             float sensor_size_y) {
    ...
    float focal_length35mm_film = std::round(focal_length * film_diagonal / sensor_diagonal);
...
bool ExifUtilsImpl::SetGpsTimestamp(const struct tm& t) {
    ...
    std::unique_ptr<ExifEntry> entry =
            AddVariableLengthEntry(EXIF_IFD_GPS, date_tag, EXIF_FORMAT_ASCII, kGpsDateStampSize,
                                   kGpsDateStampSize);
    if (!entry) {
    ...
    entry = AddVariableLengthEntry(EXIF_IFD_GPS, time_tag, EXIF_FORMAT_RATIONAL, 3,
                                   3 * sizeof(ExifRational));
    if (!entry) {
...
bool ExifUtilsImpl::SetExposureBias(int32_t ev, uint32_t ev_step_numerator,
                                    uint32_t ev_step_denominator) {
    SET_RATIONAL(EXIF_IFD_EXIF, EXIF_TAG_EXPOSURE_BIAS_VALUE, ev * ev_step_numerator,
                 ev_step_denominator);
    return true;
}
...
bool ExifUtilsImpl::SetShutterSpeed(float exposure_time) {
    ...
    SET_SRATIONAL(EXIF_IFD_EXIF, EXIF_TAG_SHUTTER_SPEED_VALUE,
                  static_cast<uint32_t>(shutter_speed * kRationalPrecision), kRationalPrecision);
    return true;
}
...
bool ExifUtilsImpl::SetSubjectDistance(float diopters) {
    ...
    if (diopters > kInfinityDiopters) {
        ...
        numerator = static_cast<uint32_t>(std::round(focus_distance * kRationalPrecision));
...
bool ExifUtilsImpl::GenerateApp1(unsigned char* thumbnail_buffer, uint32_t size) {
    DestroyApp1();
...
bool ExifUtilsImpl::SetFromMetadata(const CameraMetadata& metadata, size_t image_width,
                                    size_t image_height) {
    ...
    if (ret == OK) {
        ...
        ret = metadata.Get(ANDROID_CONTROL_POST_RAW_SENSITIVITY_BOOST, &post_raw_sens_entry);
