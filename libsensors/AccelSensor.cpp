...
int AccelSensor::readEvents(sensors_event_t *data, int count) {
    ...
    if (mBatchEnabled & (0x01 << ID_A)) {
        ...
        if (events) {
            memcpy(data, &mFifoPendingEvent[MMA8X5X_FIFO_SIZE - events],
                   sizeof(sensors_event_t) * events);
            mFifoCount -= events;
        ...
        if (mFlushed &
            (0x01
             << ID_A)) { /*if batch mode disable , send flush META_DATA_FLUSH_COMPLETE immediately*/
            memset(&sensor_event, 0, sizeof(sensor_event));
            sensor_event.version = META_DATA_VERSION;
            sensor_event.type = SENSOR_TYPE_META_DATA;
            sensor_event.meta_data.sensor = ID_A;
