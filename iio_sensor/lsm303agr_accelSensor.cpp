...
lsm303agr_accelSensor::lsm303agr_accelSensor(int32_t sensorHandle, ISensorsEventCallback* callback,
    ...
    mIioData = iio_data;
    mRunThread = std::thread(std::bind(&lsm303agr_accelSensor::run, this));
}

lsm303agr_accelSensor::~lsm303agr_accelSensor() {
...
void lsm303agr_accelSensor::batch(int32_t samplingPeriodNs) {
    samplingPeriodNs =
...
bool lsm303agr_accelSensor::supportsDataInjection() const {
    return mSensorInfo.flags & static_cast<uint32_t>(SensorFlagBits::DATA_INJECTION);
}

Result lsm303agr_accelSensor::injectEvent(const Event& event) {
...
void lsm303agr_accelSensor::setOperationMode(OperationMode mode) {
    std::unique_lock<std::mutex> lock(mRunMutex);
    if (mMode != mode) {
        mMode = mode;
        mWaitCV.notify_all();
...
bool lsm303agr_accelSensor::isWakeUpSensor() {
    return mSensorInfo.flags & static_cast<uint32_t>(SensorFlagBits::WAKE_UP);
}

Result lsm303agr_accelSensor::flush() {
...
void lsm303agr_accelSensor::processScanData(char* data, Event* evt) {
    unsigned int i, j;
    ...
    // lsm303agr_accelSensor scale value is 0.009806.
    evt->u.vec3.x = getChannelData(channelData, mXMap, false) * 0.009806;
    evt->u.vec3.y = getChannelData(channelData, mYMap, false) * 0.009806;
    evt->u.vec3.z = getChannelData(channelData, mZMap, false) * 0.009806;
    evt->timestamp = get_timestamp();
}

void lsm303agr_accelSensor::setupSysfsTrigger(const std::string& device_dir, uint8_t dev_num, bool enable) {
    add_trigger(device_dir, dev_num, enable);
}

void lsm303agr_accelSensor::setupHrtimerTrigger(const std::string& device_dir, uint8_t dev_num,
                                        bool enable) {
    add_hrtimer_trigger(device_dir, dev_num, enable);
}

void lsm303agr_accelSensor::activate(bool enable) {
    std::unique_lock<std::mutex> lock(mRunMutex);
...
void lsm303agr_accelSensor::run() {
    int read_size;
    int err;
    Event event;
