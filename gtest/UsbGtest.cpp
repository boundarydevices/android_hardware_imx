...
void UsbGtest()
{
     ...
     while(i<100)
     {
            ...
            sleep(sleep_delay);
            ret = gadget->setCurrentUsbFunctions(static_cast<uint64_t>(GadgetFunction::PTP),nullptr, 0);
            if(!ret.isOk())
            {
                ALOGI("Failed to switch to PTP(%s)",ret.description().c_str());
            }
            sleep(sleep_delay);
            ret = gadget->setCurrentUsbFunctions(static_cast<uint64_t>(GadgetFunction::RNDIS),nullptr, 0);
            if(!ret.isOk())
            {
