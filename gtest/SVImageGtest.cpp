...
static bool calibrationImage(float scaler, Matrix<double, 3, 3> &K, Matrix<double, 1, 4> &D,
                             void *pixels, void *blended_pixels, uint32_t width, uint32_t height,
                             uint32_t stride, void *output_buf, size_t outsize);
        ...
        for (int u = 0; u < out_width; u++) {
            ...
            if (u_distorted >= 0 && v_distorted >= 0 && u_distorted < width &&
                ...
                memcpy(pbuf + out_stride * v + u * bpp,
                       (char *)pixels + stride * (int)v_distorted + (int)u_distorted * bpp, bpp);

                if (blended_pixels != nullptr) {
                    int green_color = 3 | 252 >> 8 | 119 >> 16;
                    int color = *(int *)((char *)blended_pixels + stride * (int)v_distorted +
                                         (int)u_distorted * bpp);
                    int blended_color = color * (1 - 0.5) + 0.5 * green_color;
                    *(int *)((char *)blended_pixels + stride * (int)v_distorted +
                             (int)u_distorted * bpp) = blended_color;
                }
            } else {
...
static void getHomography(Matrix<double, 3, 3> &Kb, Vector3d &bR, Vector3d &bT, Vector3d &cR,
                          Vector3d &cT, Matrix<double, 3, 3> &Kc,
...
static void updateLUT(int width, int height, int bwidth, int bheight,
                      vector<Matrix<double, 3, 3>> &homographies, shared_ptr<PixelMap> &LUT_ptr) {
            ...
            for (uint32_t bu = 0; bu < bwidth; bu++) {
                ...
                if (cv >= 0 && cu >= 0 && cu < width && cv < height) {
                    ...
                    if (pMap->index0 == -1) {
                        ...
                        cout << "3 region:index=" << index << ", index0= " << pMap->index0
                             << ", index1=" << pMap->index1 << endl;
                    }
                }
            }
        }
...
static void generateBirdView(int /*width*/, int /*height*/, int stride, int bwidth, int bheight,
                             int bstride, shared_ptr<PixelMap> &LUT_ptr,
    ...
    for (uint32_t bv = 0; bv < bheight; bv++) {
        for (uint32_t bu = 0; bu < bwidth; bu++) {
            ...
            if ((pMap->index0 >= 0) && (pMap->index1 >= 0)) {
                ...
                unsigned int color0 =
                        *(int *)((char *)undistort0 + stride * pMap->v0 + pMap->u0 * bpp);
                unsigned int color1 =
                        *(int *)((char *)undistort1 + stride * pMap->v1 + pMap->u1 * bpp);
                float alpha = 0.5;
                ...
                color0 = (R << 16) | (G << 8) | B;
                *(int *)((char *)birdeye_outbuf + bstride * ((int)bv) + ((int)bu) * bpp) = color0;
                ...
                unsigned int color0 =
                        *(int *)((char *)undistort0 + stride * pMap->v0 + pMap->u0 * bpp);
                *(int *)((char *)birdeye_outbuf + bstride * bv + bu * bpp) = color0;
            }
...
static void generateBirdView(int width, int height, int stride, int bwidth, int bheight,
                             int bstride, vector<Matrix<double, 3, 3>> &homographies,
                             vector<shared_ptr<char>> &undistorts,
                             shared_ptr<char> &birdeye_outbuf_ptr) {
    int *buf_mapper = nullptr;
    int bpp = 4;
    shared_ptr<int> buf_mapper_ptr(new int[bwidth * bheight], std::default_delete<int[]>());
                ...
                if (bv >= 0 && bu >= 0 && bu < bwidth && bv < bheight) {
                    ...
                    if (*all_mapper == 0)
                        memcpy((char *)birdeye_outbuf + bstride * ((int)bv) + ((int)bu) * bpp,
                               (char *)undistort + stride * (int)v + (int)u * bpp, bpp);
                    else if (*current_mapper == 0) {
                        unsigned int color = *(int *)((char *)birdeye_outbuf + bstride * ((int)bv) +
                                                      ((int)bu) * bpp);
                        unsigned int newcolor =
                                *(int *)((char *)undistort + stride * (int)v + (int)u * bpp);
                        float alpha = 0.5;
                        unsigned char R = alpha * (color & 0xff0000 >> 16) +
                                (1 - alpha) * ((newcolor & 0xff0000) >> 16);
                        unsigned char G = alpha * (color & 0xff00 >> 8) +
                                (1 - alpha) * ((newcolor & 0xff00) >> 8);
            ...
            if (all_mapper == 0) {
                ...
                for (int s = 1; s < maxs; s++) {
                    if (buf_mapper[(v - s) * bwidth + (u - s)] != 0) {
                        color = *(int *)((char *)birdeye_outbuf + bstride * (v - s) +
                                         (u - s) * bpp);
                        R += (color & 0xff0000 >> 16);
                        G += (color & 0xff00 >> 8);
                        B += (color & 0xff);
...
static void initCameraParameters(vector<Vector3d> &evsRotations, vector<Vector3d> &evsTransforms,
...
#ifdef ENABLE_IMX_SV
    evsRotations = {{1.81080639, -0.01254808, 0.22454604},
                    {1.56411997, 1.69873618, -0.66798416},
                    {-0.12984119, 2.49551644, -1.74079733},
                    {1.65667285, -1.56078896, 1.33886394}};
    evsTransforms = {{0.00236236, 0.33980279, -0.05790999},
                     {-0.05558801, 0.38353284, 0.03021416},
                     {-0.02859015, 0.33999272, -0.05377325},
                     {-0.04265574, 0.36310098, -0.00825252}};
    Matrix<double, 3, 3> K0;
    K0 << 435.92451924, 0.0, 646.57033235, 0.0, 434.35194308, 390.30687001, 0.0, 0.0, 1.0;
    Ks.push_back(K0);
    Matrix<double, 3, 3> K1;
    K1 << 435.92451924, 0.0, 646.57033235, 0.0, 434.35194308, 390.30687001, 0.0, 0.0, 1.0;
    Ks.push_back(K1);
    Matrix<double, 3, 3> K2;
    K2 << 435.92451924, 0.0, 646.57033235, 0.0, 434.35194308, 390.30687001, 0.0, 0.0, 1.0;
    Ks.push_back(K2);
    Matrix<double, 3, 3> K3;
    K3 << 435.92451924, 0.0, 646.57033235, 0.0, 434.35194308, 390.30687001, 0.0, 0.0, 1.0;
    Ks.push_back(K3);

    Matrix<double, 1, 4> D0;
    D0 << -0.017278829037307854, -0.008386205507497259, 0.007214656916952731, -0.003014981423176122;
    Ds.push_back(D0);
    Matrix<double, 1, 4> D1;
    D1 << -0.017278829037307854, -0.008386205507497259, 0.007214656916952731, -0.003014981423176122;
    Ds.push_back(D1);
    Matrix<double, 1, 4> D2;
    D2 << -0.017278829037307854, -0.008386205507497259, 0.007214656916952731, -0.003014981423176122;
    Ds.push_back(D2);
    Matrix<double, 1, 4> D3;
    D3 << -0.017278829037307854, -0.008386205507497259, 0.007214656916952731, -0.003014981423176122;
    Ds.push_back(D3);
#else
    evsRotations = {{2.26308, 0.0382788, -0.0220549},
                    {1.67415, -1.74075, 0.789399},
                    {-0.106409, -2.83697, 1.28629},
                    {1.63019, 1.76475, -0.827941}};
    evsTransforms = {{-7.8028875403817685e-02, 1.4537396465103221e+00, -8.4197165554645001e-02},
    ...
    Matrix<double, 3, 3> K0;
    K0 << 608.0026093794693, 0.0, 968.699544102168, 0.0, 608.205469489769, 476.38843298898996, 0.0,
            0.0, 1.0;
    Ks.push_back(K0);
    Matrix<double, 3, 3> K1;
    K1 << 607.8691721095306, 0.0, 975.5686146375716, 0.0, 608.0112887189435, 481.1938786570715, 0.0,
            0.0, 1.0;
    Ks.push_back(K1);
    Matrix<double, 3, 3> K2;
    K2 << 608.557299289448, 0.0, 960.1949354417656, 0.0, 608.8093878512448, 474.74744054048256, 0.0,
            0.0, 1.0;
    Ks.push_back(K2);
    Matrix<double, 3, 3> K3;
    K3 << 608.1221963545495, 0.0, 943.6280444638576, 0.0, 608.0523818661524, 474.8564698210861, 0.0,
            0.0, 1.0;
    Ks.push_back(K3);

    Matrix<double, 1, 4> D0;
    D0 << -0.03711481733589263, -0.0014805627895442888, -0.00030212056866592464,
            -0.00020149538570397933;
    Ds.push_back(D0);
    Matrix<double, 1, 4> D1;
    D1 << -0.040116809827977926, 0.0028769489398543014, -0.002651039958977229,
            0.00024260630476736675;
    Ds.push_back(D1);
    Matrix<double, 1, 4> D2;
    D2 << -0.03998488563470043, 0.00247866869091033888, -0.002354736769480817,
            0.00018369619088506146;
    Ds.push_back(D2);
    Matrix<double, 1, 4> D3;
    D3 << -0.038096507459563965, 0.0004008114278766646, -0.0013549275607082035,
...
protected:
    ...
    virtual void SetUp() {
        Kb << 607.8691721095306, 0.0, 975.5686146375716, 0.0, 608.0112887189435, 481.1938786570715,
                0.0, 0.0, 1.0;

        initCameraParameters(evsRotations, evsTransforms, Ks, Ds);
        char input[128];
        memset(input, 0, sizeof(input));
    ...
    virtual void ShowImage(uint32_t width, uint32_t height, void *pixels) {
        mSurfaceControl = mComposerClient->createSurface(String8("ImxSVSurface"), width, height,
        ...
        ASSERT_EQ(NO_ERROR, native_window_dequeue_buffer_and_wait(anw.get(), &anb));
        ...
        buf->unlock();
        ASSERT_EQ(NO_ERROR, anw->queueBuffer(anw.get(), buf->getNativeBuffer(), -1));
        sleep(3);
    }
    ...
    bool birdeye2DSurround(float bscaler, float bird_height, uint32_t bwidth, uint32_t bheight,
                           uint32_t bstride, shared_ptr<char> &birdeye_outbuf_ptr,
                           bool lut = false) {
        bool retValue;
        Vector3d birdeyeR(0, M_PI, 0);
        Vector3d birdeyeT(0, 0, bird_height);

        if (evsRotations.size() != 4 || evsTransforms.size() != 4 || Ks.size() != 4 ||
            Ds.size() != 4) {
        ...
        for (int index = 0; index < evsRotations.size(); index++) {
            ...
            Matrix<double, 3, 3> homography;
            getHomography(Kb, birdeyeR, birdeyeT, r, t, K, &homography);
        ...
        if (!lut)
            generateBirdView(mWidth, mHeight, mStride, bwidth, bheight, bstride, homographies,
                             undistorts, birdeye_outbuf_ptr);
        else {
            updateLUT(mWidth, mHeight, bwidth, bheight, homographies, LUT_ptr);
            generateBirdView(mWidth, mHeight, mStride, bwidth, bheight, bstride, LUT_ptr,
                             undistorts, birdeye_outbuf_ptr);
        }
    ...
    bool flat2DSurround(float pw, float ph, uint32_t flatw, uint32_t flath, uint32_t fstride,
                        char *flat_outbuf) {
        int bpp = 4;
                ...
                for (uint32_t u = 0; u < flatw; u++) {
                    ...
                    if (u_distorted >= 0 && v_distorted >= 0 && u_distorted < mWidth &&
                        ...
                        if (pMap->index0 == -1) {
                            ...
                            auto fov0 = pMap->fov0;
                            auto alpha0 =
                                    (threshhold - fov0) / (threshhold - fov0 + threshhold - fov);
        ...
        for (uint32_t v = 0; v < flath; v++) {
            for (uint32_t u = 0; u < flatw; u++) {
                ...
                if ((pMap->index0 >= 0) && (pMap->index1 >= 0)) {
                    ...
                    unsigned int color0 =
                            *(int *)((char *)distort0 + mStride * pMap->v0 + pMap->u0 * bpp);
                    unsigned int color1 =
                            *(int *)((char *)distort1 + mStride * pMap->v1 + pMap->u1 * bpp);
                    // float alpha = 0.5;
...
TEST_F(ImxSV2DTest, SVLibFlatSurroundW16H12) {
    ...
    shared_ptr<char> flat_outbuf_ptr(new char[fsize], std::default_delete<char[]>());
    ...
    ImxSV2DParams sv2DParams = ImxSV2DParams(Size2dInteger(mWidth, mHeight),
                                             Size2dInteger(flatw, flath), Size2dFloat(pw, ph));
    ASSERT_TRUE(imx2DSV->SetConfigs(sv2DParams, evsRotations, evsTransforms, Ks, Ds));
    ...
    AndroidBitmapFormat format = ANDROID_BITMAP_FORMAT_RGBA_8888;
    ASSERT_TRUE(encoderImage(flatw, flath, fstride, format, flat_outbuf, output));
...
TEST_F(ImxSV2DTest, BEyeSurroundS2H4LUT) {
    ...
    shared_ptr<char> birdeye_outbuf_ptr(new char[birdeyesize], std::default_delete<char[]>());
...
TEST_F(ImxSV2DTest, RotationVector) {
    ...
    for (int index = 0; index < evsRotations.size(); index++) {
        ...
        cout << "Camera " << index << " angle with " << index_next << " = "
             << acos(dot / normal / rnextnormal) / M_PI << "pi" << endl;

        auto cross = r.cross(rnext);
        cout << "Camera " << index << " cross with " << index_next << " = \n" << cross << endl;
        cout << "Camera " << index << " angle with " << index_next << " = "
             << asin(cross.norm() / normal / rnextnormal) / M_PI << "pi" << endl;
    }
}
...
protected:
    virtual void SetUp() {
#ifdef ENABLE_IMX_SV
        K << 435.92451924, 0.0, 646.57033235, 0.0, 434.35194308, 390.30687001, 0.0, 0.0, 1.0;

        D << -0.017278829037307854, -0.008386205507497259, 0.007214656916952731,
                -0.003014981423176122;
#else
        K << 608.0026093794693, 0.0, 968.699544102168, 0.0, 608.205469489769, 476.38843298898996,
                0.0, 0.0, 1.0;

        D << -0.03711481733589263, -0.0014805627895442888, -0.00030212056866592464,
                -0.00020149538570397933;
#endif
...
protected:
    virtual void SetUp() {
        ...
        D << -0.017278829037307854, -0.008386205507497259, 0.007214656916952731,
                -0.003014981423176122;
#else
        K << 421.05641911742293, 0.0, 623.1233368341417, 0.0, 435.3963639225523, 381.33950935388606,
                0.0, 0.0, 1.0;
        D << -0.03270810830898023, 0.058951819094913656, -0.07417478474484326, 0.030368291431475524;
#endif
