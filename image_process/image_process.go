...
func init() {
	android.RegisterModuleType("imx_image_process_defaults", imageProcessDefaultsFactory)
}

func imageProcessDefaultsFactory() android.Module {
	module := cc.DefaultsFactory()
	android.AddLoadHook(module, imageProcessDefaults)
	return module
}

func imageProcessDefaults(ctx android.LoadHookContext) {
	type props struct {
		Target struct {
			Android struct {
				Enabled  *bool
