...
ExternalCameraConfig ExternalCameraConfig::loadFromCfg(const char* cfgPath) {
    ...
    if (minStreamSize == nullptr) {
        ...
        ret.minStreamSize = {static_cast<int32_t>(
    ...
    while (row != nullptr) {
        ...
        FpsLimitation limit =
                {{/* width */ static_cast<int32_t>(row->UnsignedAttribute("width", /*Default*/ 0)),
                  /* height */
                  static_cast<int32_t>(row->UnsignedAttribute("height", /*Default*/ 0))},
...
static int formatConvertSrcNV12(const YCbCrLayout& in, const YCbCrLayout& out, Size sz,
                                uint32_t format) {
...
static int formatConvertSrcI420(const YCbCrLayout& in, const YCbCrLayout& out, Size sz,
                                uint32_t format) {
    int ret = 0;
    switch (format) {
        case V4L2_PIX_FMT_NV21:
            ret = libyuv::I420ToNV21(static_cast<uint8_t*>(in.y), static_cast<int32_t>(in.yStride),
                                     static_cast<uint8_t*>(in.cb), static_cast<int32_t>(in.cStride),
                                     static_cast<uint8_t*>(in.cr), static_cast<int32_t>(in.cStride),
                                     static_cast<uint8_t*>(out.y),
                                     static_cast<int32_t>(out.yStride),
                                     static_cast<uint8_t*>(out.cr),
                                     static_cast<int32_t>(out.cStride),
                                     static_cast<int32_t>(sz.width),
                                     static_cast<int32_t>(sz.height));
            if (ret != 0) {
            ...
            break;
        case V4L2_PIX_FMT_NV12:
            ret = libyuv::I420ToNV12(static_cast<uint8_t*>(in.y), static_cast<int32_t>(in.yStride),
                                     static_cast<uint8_t*>(in.cb), static_cast<int32_t>(in.cStride),
                                     static_cast<uint8_t*>(in.cr), static_cast<int32_t>(in.cStride),
                                     static_cast<uint8_t*>(out.y),
                                     static_cast<int32_t>(out.yStride),
...
int encodeJpeg(uint32_t fourcc, const Size& inSz, const YCbCrLayout& inLayout, int jpegQuality,
    ...
    /* Configure sampling factors.
       For YU12 and NV12, the sampling factor is JPEG subsampling 420;
       For NV16 and YUYV, the sampling factor is JPEG subsampling 422;
    */
    ...
    uint8_t y_step = 1;
    if (fourcc == V4L2_PIX_FMT_NV16 || fourcc == V4L2_PIX_FMT_YUYV) {
    ...
    JSAMPLE y_plane[batchSize][cinfo.image_width];
    JSAMPLE u_plane[batchSize][cinfo.image_width / 2];
    JSAMPLE v_plane[batchSize][cinfo.image_width / 2];
    JSAMPROW y_rows[batchSize];
    JSAMPROW u_rows[batchSize];
    JSAMPROW v_rows[batchSize];
    ...
    while (cinfo.next_scanline < cinfo.image_height) {
        for (JDIMENSION i = 0; i < batchSize; ++i) {
            if ((cinfo.next_scanline + i) >= cinfo.image_height)
                break;
            auto offset = (cinfo.next_scanline + i) * inLayout.yStride;
            auto offsetcb = (cinfo.next_scanline + i) / cVSubSampling * inLayout.cStride;
            for (JDIMENSION j = 0; j < cinfo.image_width; j += 2) {
                y_plane[i][j] = py[offset + j * y_step];
                y_plane[i][j + 1] = py[offset + j * y_step + y_step];

                u_plane[i][(j / 2)] = pcb[(offsetcb + (j / 2) * inLayout.chromaStep)];
                v_plane[i][(j / 2)] = pcr[(offsetcb + (j / 2) * inLayout.chromaStep)];
            }
        }

        uint32_t done = jpeg_write_raw_data(&cinfo, rows, batchSize);
        if (done != batchSize) {
