...
void ConfigManager::printElementNames(const XMLElement *rootElem, string prefix) const {
        ...
        if (ConfigManagerUtil::convertToMetadataTag(curElem->FindAttribute("name")->Value(), tag)) {
            switch (tag) {
                ...
                case ANDROID_LENS_INTRINSIC_CALIBRATION: {
                    ...
                    aCamera->cameraMetadata.insert_or_assign(tag, make_pair(data, count));
                ...
                case ANDROID_REQUEST_AVAILABLE_CAPABILITIES: {
                    ...
                    if (ConfigManagerUtil::convertToCameraCapability(curElem->FindAttribute("value")
                                                                             ->Value(),
                                                                     *data)) {
                        aCamera->cameraMetadata.insert_or_assign(tag, make_pair((void *)data, 1));
                ...
                case ANDROID_LOGICAL_MULTI_CAMERA_PHYSICAL_IDS: {
                    ...
                    aCamera->cameraMetadata.insert_or_assign(tag, make_pair((void *)data, len + 1));
...
bool ConfigManager::constructCameraMetadata(CameraInfo *aCamera, const size_t totalEntries,
    ...
    if (!find_camera_metadata_entry(reinterpret_cast<camera_metadata_t *>(aCamera->characteristics),
                                    ANDROID_SCALER_AVAILABLE_STREAM_CONFIGURATIONS, &streamCfgs)) {
        ALOGE("have no ANDROID_SCALER_AVAILABLE_STREAM_CONFIGURATIONS in CameraMetadata");
    }
...
bool ConfigManager::readConfigDataFromBinary() {
    ...
    for (auto cidx = 0; cidx < ngrps; ++cidx) {
        ...
        string cameraId = *(reinterpret_cast<string *>(p));
        p += sizeof(string);
        ...
        for (auto idx = 0; idx < sz; ++idx) {
            CameraCtrl temp = *ptr++;
            aCamera->controls.emplace(temp.cid, make_tuple(temp.min, temp.max, temp.step));
        }
...
bool ConfigManager::writeConfigDataToBinary() {
    ...
    for (auto &&[camId, camInfo] : mCameraGroups) {
        ...
        outFile.write(reinterpret_cast<const char *>(&camId), sizeof(string));
        ...
        for (auto &&[ctrl, range] : camInfo->controls) {
            ...
            outFile.write(reinterpret_cast<const char *>(&get<0>(range)), sizeof(int32_t));
            outFile.write(reinterpret_cast<const char *>(&get<1>(range)), sizeof(int32_t));
            outFile.write(reinterpret_cast<const char *>(&get<2>(range)), sizeof(int32_t));
        }
