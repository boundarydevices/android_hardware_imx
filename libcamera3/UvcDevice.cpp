...
//----------------------UvcDevice--------------------
Camera* UvcDevice::newInstance(int32_t id, char* name, int32_t facing, int32_t orientation,
    ...
    device = new UvcDevice(id, facing, orientation, path, cam_copy_hw, cam_csc_hw, hw_jpeg_enc,
                           true, cam_metadata);

    return device;
}

UvcDevice::UvcDevice(int32_t id, int32_t facing, int32_t orientation, char* path, CscHw cam_copy_hw,
                     CscHw cam_csc_hw, const char* hw_jpeg_enc, bool createStream,
...
status_t UvcDevice::initSensorStaticData() {
    ...
    while (ret == 0) {
        ...
        ALOGV("index:%d,ret:%d, format:%c%c%c%c", index, ret, vid_fmtdesc.pixelformat & 0xFF,
              (vid_fmtdesc.pixelformat >> 8) & 0xFF, (vid_fmtdesc.pixelformat >> 16) & 0xFF,
    ...
    while (ret == 0) {
        ...
        ALOGI("enum frame size w:%d, h:%d", vid_frmsize.discrete.width,
        ...
        ALOGI("vid_frmval denominator:%d, numeraton:%d", vid_frmval.discrete.denominator,
    ...
    ALOGI("UvcDevice, mFocalLength:%f, mPhysicalWidth:%f, mPhysicalHeight %f", mFocalLength,
