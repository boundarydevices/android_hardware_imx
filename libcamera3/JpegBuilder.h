...
struct JpegParams {
    JpegParams(uint8_t *uSrc, uint8_t *uSrcPhy, int srcSize, uint8_t *uDst, int dstSize,
               int quality, int inWidth, int inHeight, int outWidth, int outHeight, int format)
          : src(uSrc),
            srcPhy(uSrcPhy),
            src_size(srcSize),
            dst(uDst),
...
private:
    status_t insertElement(uint16_t tag, uint32_t val1, uint32_t val2, uint32_t val3, uint32_t val4,
                           uint32_t val5, uint32_t val6, char *strVal);
