...
::ndk::ScopedAStatus OemLock::setOemUnlockAllowedByDevice(bool in_allowed) {
    LOG(INFO) << "Running OemLock_Software::setOemUnlockAllowedByDevice: " << in_allowed;

    fbmiscError rc = mFbmiscDataBlockLock.writeDeviceUnlockPermission(in_allowed ? 1 : 0);
    if (rc != fbmiscError::FBMISC_ERROR_NONE) {
...
::ndk::ScopedAStatus OemLock::isOemUnlockAllowedByDevice(bool *out_allowed) {
    ...
    LOG(INFO) << "Running OemLock_Software::isOemUnlockAllowedByDevice";

    fbmiscError rc = mFbmiscDataBlockLock.readDeviceUnlockPermission(&status);
    if (rc != fbmiscError::FBMISC_ERROR_NONE) {
