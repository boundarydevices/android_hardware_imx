...
using fsl_imx8::SoftGateKeeperDevice;

struct fsl_imx8_gatekeeper_device {
    gatekeeper_device device;
    SoftGateKeeperDevice *s_gatekeeper;
};

static fsl_imx8_gatekeeper_device s_device;
...
static int enroll(const struct gatekeeper_device *dev __unused, uint32_t uid,
    ...
    SoftGateKeeperDevice *s_gatekeeper = ((fsl_imx8_gatekeeper_device*)(dev))->s_gatekeeper;
    ALOGV("called %s with gate keeper %p device %p\n", __func__, s_gatekeeper, dev);
...
static int fsl_imx8_gatekeeper_open(const hw_module_t *module, const char *name,
        hw_device_t **device) {
