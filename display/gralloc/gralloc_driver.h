...
public:
    ...
    int32_t dmabuf_allocate(gralloc_buffer_descriptor *desc, gralloc_handle **out_handle);
    bool allocate_from_gpu_gralloc(int32_t pixel_format, uint64_t usage, uint64_t flags);
    int32_t allocate(struct gralloc_buffer_descriptor *descriptor, native_handle_t **out_handle);
    ...
    int32_t lock(buffer_handle_t handle, int32_t acquire_fence, bool close_acquire_fence,
                 const struct rectangle *rect, uint64_t usage, uint8_t *addr[DRV_MAX_PLANES]);
    ...
    uint64_t get_android_usage(gralloc_handle_t hnd) const { return hnd->usage; }
    uint32_t get_layer_count(gralloc_handle_t hnd) const { return hnd->layer_count; }
    uint32_t get_drm_format(gralloc_handle_t hnd) const { return hnd->drm_format; }
...
private:
    ...
    // VSI GPU Gralloc
    alloc_device_t *mGPUAlloc = nullptr;
    gralloc_module_t *mGPUModule = nullptr;
