...
Return<Error> NxpMapper::validateBufferSize(void* rawHandle, const BufferDescriptorInfo& descriptor,
    ...
    */
    ...
    if (descriptor.width != static_cast<uint32_t>(memHandle->width)) {
        ALOGE("%s Width mismatch (%d vs %d).", __func__, descriptor.width, memHandle->width);
        return Error::BAD_VALUE;
    }

    if (descriptor.height != static_cast<uint32_t>(memHandle->height)) {
        ALOGE("%s Height mismatch (%d vs %d).", __func__, descriptor.height, memHandle->height);
        return Error::BAD_VALUE;
    }

    if (stride != static_cast<uint32_t>(memHandle->stride)) {
...
Return<void> NxpMapper::get(gralloc_handle_t memHandle, const MetadataType& metadataType,
    ...
    if (metadataType == android::gralloc4::MetadataType_BufferId) {
        status = android::gralloc4::encodeBufferId(mDriver->get_id(memHandle), &encodedMetadata);
    } else if (metadataType == android::gralloc4::MetadataType_Name) {
        const char* name = (const char*)(grallocMetadata->name);
        status = android::gralloc4::encodeName(name, &encodedMetadata);
    } else if (metadataType == android::gralloc4::MetadataType_Width) {
        status = android::gralloc4::encodeWidth(mDriver->get_width(memHandle), &encodedMetadata);
    } else if (metadataType == android::gralloc4::MetadataType_Height) {
        status = android::gralloc4::encodeHeight(mDriver->get_height(memHandle), &encodedMetadata);
        ...
        PixelFormat pixelFormat = static_cast<PixelFormat>(mDriver->get_android_format(memHandle));
        ...
        status = android::gralloc4::encodePixelFormatModifier(mDriver->get_format_modifier(
                                                                      memHandle),
                                                              &encodedMetadata);
    } else if (metadataType == android::gralloc4::MetadataType_Usage) {
        uint64_t usage = static_cast<uint64_t>(mDriver->get_android_usage(memHandle));
        ...
        status = android::gralloc4::encodeAllocationSize(mDriver->get_total_size(memHandle),
                                                         &encodedMetadata);
    } else if (metadataType == android::gralloc4::MetadataType_ProtectedContent) {
        uint64_t hasProtectedContent =
                mDriver->get_android_usage(memHandle) & BufferUsage::PROTECTED ? 1 : 0;
