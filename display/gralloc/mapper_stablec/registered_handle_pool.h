...
public:
    static RegisteredHandlePool &get_instance();

    RegisteredHandlePool(const RegisteredHandlePool &) = delete;
    RegisteredHandlePool(RegisteredHandlePool &&) = delete;

    void operator=(const RegisteredHandlePool &) = delete;
    void operator=(RegisteredHandlePool &&) = delete;
...
private:
    static RegisteredHandlePool instance;

    std::mutex mutex;
    std::unordered_set<buffer_handle_t> bufPool;

    RegisteredHandlePool() = default;
};
