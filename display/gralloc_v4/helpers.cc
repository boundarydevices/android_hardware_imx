...
{
	if (format == FORMAT_NV12_TILED ||
		format == FORMAT_NV12_G1_TILED ||
		format == FORMAT_NV12_G2_TILED ||
		format == FORMAT_NV12_G2_TILED_COMPRESSED ||
		format == FORMAT_P010_TILED ||
				   ...
				   uint32_t format, uint32_t padding[DRV_MAX_PLANES])
{
	...
	for (p = 0; p < num_planes; p++) {
		...
		hnd->sizes[p] =
		    drv_size_from_format(format, hnd->strides[p], aligned_height, p) +
		    padding[p];
